...
try:
    ...
    print('cv2 is unavailable, Hutil.img can not be used.')
import numpy as np
import logging
import math
import os
...
def get_contour_region_in_min_area_rect(img, cnt):
    ...
    new_corners = Hutil.dtype.int(np.transpose(new_corners))
...
#     """
#     Hutil.test.assert_equal(np.shape(box), (4, 2))
#     cnt = to_contours(box)
...
def random_color_3():
    c = Hutil.rand.randint(low = 0, high = 255, shape = (3, ))
#     c = np.uint8(c)
    return c
