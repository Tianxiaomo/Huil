...
def ls(path = '.', suffix = None):
    ...
    filtered = []
    for f in files:
        if Hutil.str.ends_with(f, suffix, ignore_case = True):
            filtered.append(f)
...
def search(pattern, path, file_only = True):
    ...
    """
    path = get_absolute_path(path)
    pattern_here = Hutil.io.join_path(path, pattern)
    targets = []
    
    # find matchings in current directory
    candidates = find_files(pattern_here)
    for can in candidates:
        if Hutil.io.is_dir(can) and file_only:
            continue
        else:
            targets.append(can)
    ...
    files = ls(path)
    for f in files:
        fpath = Hutil.io.join_path(path, f)
        if is_dir(fpath):
