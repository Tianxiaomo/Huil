...
#         x = np.reshape(x, np.prod(x.shape))
    ...
    if save:
        if save_path is None:
            raise ValueError
        path = Hutil.io.join_path(save_path, title + '.png')
        save_image(path)
    if show:
        plt.show()
        #Hutil.img.imshow(title, path, block = block)        

def plot_solver_data(solver_path):
    data = Hutil.io.load(solver_path)
...
def get_random_line_style():
    colors = ['r', 'g', 'b']
    line_types = ['-']#, '--', '-.', ':']
    idx = Hutil.rand.randint(len(colors))
    color = colors[idx]
    idx = Hutil.rand.randint(len(line_types))
