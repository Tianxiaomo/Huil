...
try:
    ...
    print("tensorflow is not installed, Hutil.tf can not be used.")
...
# tf.train.latest_checkpoint
    import Hutil
    path = Hutil.io.get_absolute_path(path)
    if Hutil.io.is_dir(path):
...
def get_iter(ckpt):
    import Hutil
    iter_ = int(Hutil.str.find_all(ckpt, '.ckpt-\d+')[0].split('-')[-1])
    return iter_
...
def get_init_fn(checkpoint_path, train_dir, ignore_missing_vars = False,
    ...
    """
    code from github/SSD-tensorflow/tf_Hutils.py
    ...
    """
    import Hutil
    if Hutil.str.is_none_or_empty(checkpoint_path):
        return None
...
def Print(tensor, data, msg = '', file = None, mode = 'w'):
    ...
    import Hutil
    def np_print(*args):
        if Hutil.str.contains(msg, '%'):
            message = msg%tuple(args)
        else:
            message = msg + ' %'*len(args)%tuple(args)
        if file is not None:
            file_path = Hutil.io.get_absolute_path(file)
...
def min_area_rect(xs, ys):
    import Hutil
    rects = tf.py_func(Hutil.img.min_area_rect, [xs, ys], xs.dtype)
